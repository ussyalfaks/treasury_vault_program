...
#[tokio::test]
async fn execute_sol_payout_ix_success() {
	...
	let recipient_address_pubkey = Pubkey::new_unique();
	...
	let ix = treasury_vault_ix_interface::execute_sol_payout_ix_setup(
		&fee_payer_keypair,
		treasury_pda,
		recipient_pda,
		payout_schedule_pda,
		recipient_address_pubkey,
		schedule_id,
		&treasury_seed_name,
		recent_blockhash,
	);
